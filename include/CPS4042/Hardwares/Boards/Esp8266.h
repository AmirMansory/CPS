...
public:
    ...
    // ==================== بخش اضافه شده ====================
    private:
        Byte m_slaveAddress;        // آدرس سنسور هدف
        Byte m_rxBuffer[3];          // بافر دریافت داده (3 بایت: 2 بایت داده + 1 بایت checksum)
        int m_rxIndex;               // اندیس فعلی در بافر دریافت
        bool m_waitingForData;       // وضعیت انتظار برای دریافت داده
        bool m_ackReceived;          // وضعیت دریافت ACK از سنسور
        enum State { IDLE, SEND_ADDRESS, WAIT_ACK, READ_DATA, STOP } m_state;
        // ====================================================
    ...
    public:
        ...
        // ==================== بخش اضافه شده ====================
        {
            ...
            std::cout << "[ESP8266 I2C] Initialized with slave address: 0x" 
        ...
        {
            if (m_state == IDLE) {
                m_state = SEND_ADDRESS;
                std::cout << "[ESP8266 I2C] Sending address: 0x" 
        ...
        // ==================== بخش اضافه شده ====================
        {
            if (m_state == READ_DATA && m_rxIndex < 3) {
                return m_rxBuffer[m_rxIndex++];
            }
            return 0;
        }
        // ====================================================
        ...
        // ==================== بخش اضافه شده ====================
        {
            switch (m_state) {
                ...
                case WAIT_ACK:
                    if (m_ackReceived) {
                        std::cout << "[ESP8266 I2C] ACK received, reading data..." << std::endl;
                        m_state = READ_DATA;
                        m_rxIndex = 0;
                    }
                    break;
                ...
                case READ_DATA:
                    if (m_rxIndex >= 3) {
                        std::cout << "[ESP8266 I2C] All 3 bytes received" << std::endl;
                        m_state = STOP;
                    }
                    break;
        ...
        void receiveData(Byte data, int index) {
            if (m_state == READ_DATA && index < 3) {
                m_rxBuffer[index] = data;
                std::cout << "[ESP8266 I2C] Received byte " << index + 1 
                          << ": " << (int)data << std::endl;
            }
        }
        
        bool isWaitingForData() const {
            return (m_state == READ_DATA);
        }
